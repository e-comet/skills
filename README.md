...
> Скачай для артикула 111111111 отзывы без ответа за всё время, а для 222222222 — отвеченные за июнь 2026 года.

Можно объединять разные артикулы, периоды и типы отзывов в одном запросе. По умолчанию используется юрлицо, активное в
портале продавца; другое доступное юрлицо можно указать явно. Успешные файлы возвращаются ссылками даже при частичном
результате. Файлы хранятся локально 24 часа, а их содержимое не помещается в контекст.
