# e-Comet Skills

Агентские скиллы e-Comet помогают продавцам работать с платформой Wildberries: находить данные, проверять гипотезы,
собирать операционные отчеты и автоматизировать повторяющиеся действия. Они спроектированы как небольшие и надежные
строительные блоки для сложных рабочих процессов продавца.

Скиллы устанавливаются как marketplace-плагин в Claude Cowork и Codex Desktop.

Вместе с плагином автоматически подключается MCP сервер e-Comet (`https://mcp.e-comet.io/mcp`) —
устанавливать его вручную не нужно. При первом обращении скилла к серверу откроется вход по
email: введите адрес и код из письма, дальше клиент продлевает доступ сам.

## Быстрый старт (установка за 30 секунд)

### Claude Cowork

1. Профиль > `Settings` > `Capabilities` > Вкл. `Allow network egress` > `Domain allowlist` > `All domains`
2. `Customize` на панели слева > `Plugins` > `Add` > `Add marketplace`
3. `Add from repository` > поле URL: `https://github.com/e-comet/skills` > `Sync`
4. Откройте marketplace `e-comet-skills` > установите плагин `e-Comet Skills`

### Codex Desktop

1. `Settings` на панели слева > `Settings` > `Configuration` > Вкл. `Allow network access`
2. `Plugins` на панели слева > `+` в правом верхнем углу > `Add marketplace`
3. Поле `Source`: `https://github.com/e-comet/skills` > `Add marketplace`
4. `Personal` > `e-Comet Skills` > `Add`

## Список скиллов

Обозначения требований:

- 🔌 `MCP` — нужен MCP сервер e-Comet (подключается автоматически с плагином, вход по email);
- 🧩 `Расширение` — нужно [браузерное расширение e-Comet](https://e-comet.io/) с введённым ключом.

Скиллы без обозначений работают сразу после установки плагина.

**wb-product-images** - получение URL фото артикулов ВБ
> Сделай HTML галерею фото артикулов ВБ 791050753 и 913357757: строка на артикул, клик открывает полный размер, стрелки переключают фото.

**wb-product-card** 🔌 🧩 - живая карточка товара ВБ: цена, рейтинг, остатки по складам и размерам, склейка
> Покажи остатки по складам и размерам для артикула 503093790.

**wb-search-by-query** 🔌 🧩 - живая поисковая выдача ВБ: топ по запросу и позиции артикулов
> На какой позиции артикул 503093790 в поиске ВБ по запросу «ковролин»?


## Ошибки / не работает скилл?

Создайте issue на github с детальным описанием:
- Название ИИ-платформы
- Последовательность шагов для воспроизведения (конкретный пример)
- Ожидание
- Реальность
