...
- **Базовая** - только внутренная аналитика по своим артикулам. Подходит, если нет возможности установить десктопное приложение ИИ-агента (например, доступен только ChatGPT или Claude через веб-браузер)
...
4. В окне `Add e-Comet to ChatGPT` нажимите `Sign in with e-Comet` и введите почту.
...
4. Убедитесь, что оба MCP servers `E-comet` и `E-comet-local` готовы к работе (справа ⚙️), иначе нажмите `Install` / `Connect` и введите почту.
